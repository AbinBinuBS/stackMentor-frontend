  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 max-w-2xl mx-auto h-[500px] flex flex-col">
      ...
      <div className="mb-4">
        ...
        <button
          className={`px-4 py-2 ${
            activeTab === 'answered' ? 'bg-purple-600 text-white' : 'bg-gray-200'
          } rounded-md focus:outline-none`}
      ...
      <div className="overflow-y-auto flex-grow">
        <div className="space-y-3">
          {filteredQuestions.map((question) => (
            <div key={question._id} className="border border-gray-200 rounded-md p-3">
              ...
              {question.isAnswered && activeTab === 'answered' && (
                <div>
                  ...
                  >
                    {showAnswerIds.includes(question._id) ? 'Hide Answer' : 'Show Answer'}
                  </button>
                  {showAnswerIds.includes(question._id) && (
                    <p className="text-sm text-green-600 my-2">Answer: {question.reply}</p>
                  )}
                  <button
