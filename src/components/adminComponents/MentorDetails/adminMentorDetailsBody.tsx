...
import { IMentorDetails } from "../../../interfaces/IAdminInterface";
...
const AdminMentorDetailsBody: React.FC = () => {
		...
		const getMentorDetails = async () => {
			try {
				const response = await apiClientAdmin.get(
					`${LOCALHOST_URL}/api/admin/getMentorDetails`,
					{
						params: { id },
					}
				);
				...
				if (response.status === 200 && response.data.message === "Success") {
					...
					setVerificationStatus(
						response.data.mentorData.isVerified ? "verified" : "pending"
					);
				} else {
					toast.error(
						response.data.message || "Failed to retrieve mentor details."
					);
				}
			} catch (error) {
	...
	const DocumentBox: React.FC<{ label: string; link: string }> = ({
		label,
		link,
	}) => (
		<div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-4">
	...
	const handleSubmit = async () => {
		try {
			const updateResponse = await apiClientAdmin.post(
				`${LOCALHOST_URL}/api/admin/updateMentorStatus`,
				{
					id,
			...
			if (
				...
				updateResponse.data.message === "Status updated successfully."
			) {
				toast.success("Mentor status updated successfully.");
				const updatedResponse = await apiClientAdmin.get(
					`${LOCALHOST_URL}/api/admin/getMentorDetails`,
					{
						params: { id },
					}
				...
				if (
					...
					updatedResponse.data.message === "Success"
				) {
					setMentorDetails(updatedResponse.data.mentorData);
					setVerificationStatus(
						updatedResponse.data.mentorData.isVerified ? "verified" : "pending"
					);
				} else {
					toast.error(
						updatedResponse.data.message || "Error fetching updated details."
					);
				}
			} else {
				toast.error(
					updateResponse.data.message || "Failed to update mentor status."
				);
			}
		} catch (error) {
			if (axios.isAxiosError(error)) {
	...
	const renderStatusOptions = () => {
		if (status === "verified") {
			return <option value="verified">Verified</option>;
		} else if (status === "rejected") {
			return <option value="rejected">Rejected</option>;
		} else {
			return (
				<>
					...
					{mentorDetails ? (
						<>
							...
							<DocumentBox
								...
								link={mentorDetails.degreeCertificate}
							/>
							<DocumentBox
								label="Experience Certificate"
								link={mentorDetails.experienceCertificate}
							/>
						</>
					) : (
						<p>Loading...</p>
