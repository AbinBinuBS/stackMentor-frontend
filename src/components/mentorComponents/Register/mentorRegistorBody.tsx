...
const MentorRegisterBody: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoading(true);
            try {
                const response = await axios.post(`${LOCALHOST_URL}/api/mentor/register`, values);
                ...
                if (response.data.message === 'OTP Send Successfully') {
                    ...
                    toast.error(response.data.message || 'An error occurred during registration. Please try again.');
                }
            } catch (error: any) {
