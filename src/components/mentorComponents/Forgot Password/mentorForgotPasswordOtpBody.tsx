...
const MentorForgotPasswordBody: React.FC = () => {
	...
	const handleResendOtp = async () => {
		try {
			...
			const response = await axios.post(
				`${LOCALHOST_URL}/api/mentor/resend-otp`,
				{ email }
			);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const response = await axios.post(
				`${LOCALHOST_URL}/api/mentor/reset-password-verify-otp`,
				{
					email,
					otp: otpString,
				}
			...
			if (response.data.message === "OTP verified successfully") {
				toast.success(response.data.message);
				navigate(
					`/mentor/forgot-password-reset?email=${encodeURIComponent(
						response.data.email
					)}`
				);
			} else {
				...
				<div className="flex flex-col items-center justify-center w-full max-w-sm">
					...
					<form onSubmit={handleSubmit} className="space-y-4 w-full">
						...
						<button
							...
							} text-white rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50`}
