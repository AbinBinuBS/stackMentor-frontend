...
const MenteeForgotPasswordBody = () => {
  ...
  const { values, handleBlur, handleChange, handleSubmit, errors, touched } = useFormik({
    ...
    onSubmit: async (value) => {
      setLoading(true);
      try {
        const response = await axios.post(`${LOCALHOST_URL}/api/mentees/forgot-password`, value);
        if(response.data.message == "Password reset link sent to your email."){
          toast.success("Otp send successfully.")
          navigate(`/forgot-password-otp?email=${encodeURIComponent(value.email)}`)
        }
    ...
    <div className="flex justify-center items-center w-full h-screen bg-gray-100">
      <div className="flex bg-white rounded-lg shadow-lg w-[700px] h-auto max-h-[800px] overflow-hidden">
        <div className="flex-shrink-0 w-1/2 bg-blue-200 flex items-center justify-center">
          <img src="/images/loginimg.jpg" alt="Forgot Password" className="object-cover w-full h-full" />
        </div>
