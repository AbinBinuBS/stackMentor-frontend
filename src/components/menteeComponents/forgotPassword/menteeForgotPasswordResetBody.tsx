  ...
  const [searchParams] = useSearchParams();
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                console.log(values)
                const response = await axios.post(`${LOCALHOST_URL}/api/mentees/reset-password-reset`, {
                email,
                values
                });
        ...
        <div className="flex justify-center items-center w-full h-screen bg-gray-100">
            <div className="flex bg-white rounded-lg shadow-lg w-[700px] h-auto max-h-[800px] overflow-hidden">
                <div className="flex-shrink-0 w-1/2 bg-blue-200 flex items-center justify-center">
                    <img src="/images/loginimg.jpg" alt="Reset Password" className="object-cover w-full h-full" />
                </div>
                        ...
                        <form className="space-y-4" onSubmit={formik.handleSubmit}>
                            ...
                            <button
                                ...
                                className={`w-full py-2 px-4 ${
                                    formik.isSubmitting ? 'bg-gray-400' : 'bg-custom-cyan hover:bg-cyan-200'
