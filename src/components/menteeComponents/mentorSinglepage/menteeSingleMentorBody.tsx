...
const handlePaymentMethodSelection = async (method: 'stripe' | 'wallet') => {
    ...
    if (method === 'stripe') {
        ...
        if (result.isConfirmed) {
            try {
                ...
                const { data } = await apiClientMentee.post(`${LOCALHOST_URL}/api/mentees/payUsingWallet`, { sessionId: selectedSession._id });
    ...
    const processPayment = async () => {
        ...
        try {
            const response = await axios.post(`${LOCALHOST_URL}/api/mentees/checkAvailable`, { sessionId: selectedSession._id });
            console.log(response);
            if (response.data.message === "Success") {
                const { data } = await apiClientMentee.post(`${LOCALHOST_URL}/api/mentees/menteePayment`, {
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                <div className="bg-white p-8 rounded-lg max-w-md w-full shadow-2xl">
                    <h2 className="text-2xl font-bold mb-6 text-center text-[#1D2B6B]">Book Mentor</h2>
                    <div className="space-y-4 mb-6">
                        <p className="text-lg">
                            <span className="font-semibold">Date:</span> {new Date(selectedSession.date).toDateString()}
                        </p>
                        <p className="text-lg">
                            <span className="font-semibold">Time:</span> {convertTo12HourFormat(selectedSession.startTime)} - {convertTo12HourFormat(selectedSession.endTime)}
                        </p>
                        <p className="text-lg">
                            <span className="font-semibold">Price:</span> ₹{selectedSession.price}
                        </p>
                    </div>
                    <div className="mb-6">
                        <CardElement
    ...
    const PaymentMethodModal = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                <div className="bg-white p-8 rounded-lg max-w-md w-full shadow-2xl">
                    <h2 className="text-2xl font-bold mb-6 text-center text-[#1D2B6B]">Choose Payment Method</h2>
                    <div className="space-y-4 mb-6">
                        <button
