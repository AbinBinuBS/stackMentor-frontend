    ...
    async error => {
        ...
        if (error.response && error.response.status === 403) {
            ...
            if (errorCode === 'ACCOUNT_INACTIVE') {
                ...
                return Promise.reject(new Error('Mentee is not verified. Please complete the verification process.'));
            }
        }
        ...
        if (error.response && error.response.status === 401 && !originalRequest._retry) {
            ...
            if (refreshToken) {
                try {
                    const { data } = await apiClientMentee.post('/auth/refresh-token', { refreshToken });
                    store.dispatch(menteeLogin({ accessToken: data.accessToken, refreshToken: data.refreshToken }));
